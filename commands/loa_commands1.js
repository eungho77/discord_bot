...
const discord = {
    ...
    select_item: async (interaction, url) => {
        ...
        if(num == 2){
            result = await discord.character_equipment(interaction, url, nickname)
        }
        if(num == 3){
            result = await discord.character_skills(interaction, url, nickname)
        }
        if(num == 4){
    ...
    character_equipment: async (interaction, url, nickname) => {
        let result = {}
        let info = await axios.get(url+'/api/info/' + encodeURI(interaction.member.nickname) + '/' + encodeURI(nickname))
        if (interaction.channelId === channelsId){
            if(info.data.search) {
                exampleEmbed.title = '장비구 / 장신구 / 아바타 목록 정보'
                exampleEmbed.description = '[' + nickname + ']님이 가지고 있는 장비 목록입니다.'
                exampleEmbed.fields = order.equipment(info.data)
            } else {
                exampleEmbed.title = '[' + nickname + ']님은 존재하지 않습니다.'
                exampleEmbed.description = info.data.content
                exampleEmbed.fields = []
    ...
    character_skills: async (interaction, url, nickname) => {
        let result = {}
        let info = await axios.get(url+'/api/info/' + encodeURI(interaction.member.nickname) + '/' + encodeURI(nickname))
        if (interaction.channelId === channelsId){
            if(info.data.search) {
                exampleEmbed.title = '스킬 정보'
                exampleEmbed.description = '[' + nickname + ']님이 가지고 있는 스킬 정보입니다.'
                exampleEmbed.fields = order.skill(info.data)
            } else {
                exampleEmbed.title = '[' + nickname + ']님은 존재하지 않습니다.'
                exampleEmbed.description = info.data.content
                exampleEmbed.fields = []
